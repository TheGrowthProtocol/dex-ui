...
const ERC20 = require("./build/Token1.json");
...
export function getWceres(address, signer) {
  return new Contract(address, WCERES.abi, signer);
}
...
export async function getBalanceAndSymbol(
  ...
  try {
    if (address === wceres_address) {
      const balanceRaw = await provider.getBalance(accountAddress);
      //const symbol = await WCERES.symbol();
      return {
...
export async function swapTokens(
  ...
  if (address1 === wceresAddress) {
    // Eth -> Token
    await routerContract.swapExactCERESForTokens(
      amountOut[1],
      tokens,
      accountAddress,
      deadline,
    ...
    } else if (address2 === wceresAddress) {
    // Token -> Eth
    await routerContract.swapExactTokensForCERES(
      amountIn,
      amountOut[1],
      tokens,
      accountAddress,
