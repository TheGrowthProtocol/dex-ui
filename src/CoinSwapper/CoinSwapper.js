...
function CoinSwapper(props) {
  ...
  const onToken1Selected = (address) => {
    ...
    // We only update the values if the user provides a token
      ...
      getBalanceAndSymbol(props.network.account, address, props.network.provider, props.network.signer, props.network.wceres.address, props.network.coins).then((data) => {
        setCoin1({
          address: address,
          symbol: data.symbol,
